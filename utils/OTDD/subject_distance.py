...
def loadData(root,classes=3,n=20):
    ...
    for i in range(classes):
        ...
        for d in data_list:
            if int(d[0])==i:
                current_num+=1
                slice_list=os.listdir(os.path.join(root,d))
                selected_slice=slice_list[int(len(slice_list)/2)]
                result.append(os.path.join(root,d,selected_slice))
                labels.append(i)
            if current_num>=n:
                break
...
if __name__=='__main__':
    ...
    DATA_DIR='/home/mist/data/end-to-end-filtered-gate/low/train'

    DATA_HE_DIR='/home/mist/data/end-to-end-filtered-gate/high/train'
    
    classes=[0,1,2]
    
    features,labels=loadData(DATA_DIR)
    features_he,labels_he=loadData(DATA_HE_DIR)
