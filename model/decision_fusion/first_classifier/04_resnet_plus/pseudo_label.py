...
def main():
    ...
    for class_index,class_name in enumerate(CLASSES_NAME):
        ...
        for i,patient_dir in enumerate(patient_list):
            ...
            predlist= test(slice_list,class_index,model)
            print('{} ----- {}'.format(i,len(patient_list)))
            result_list+=result
            
            
    with open(os.path.join(LABEL_DIR,'pseudo_label.csv'),'w')as f:
        f_csv = csv.writer(f)
...
def test(test_list, class_index , model):
    ...
    test_loader = torch.utils.data.DataLoader(dataset.pseudoDataset(test_list,class_index), 
    ...
    with torch.no_grad():
        for i,(img, img_path) in enumerate(test_loader):
            img = img.cuda()
            ...
            items=[[pred[i]]+img_path[i].split(os.path.sep)[-3:] for i in range(len(pred))]
